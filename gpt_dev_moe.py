...
class MoE(nn.Module):
    ...
    def __init__(self, n_embd, num_experts=2, topk_experts=2):
        super().__init__()
        self.num_experts = num_experts
        self.topk_experts = topk_experts
        self.experts = nn.ModuleList([
            FeedForward(n_embd) for _ in range(num_experts)
        ])
    ...
    def forward(self, x):
        B, T, C = x.shape
        x_2d = x.view(-1, C)                                                # [B*T, C]
        
        # get top-k experts
        logits = self.router(x_2d)                                          # [B*T, num_experts]
        probs = F.softmax(logits, dim=-1)
        topk_probs, topk_indices = probs.topk(self.topk_experts, dim=-1)    # [B*T, k]
        
        # create binary mask for experts
        mask = F.one_hot(topk_indices, num_classes=self.num_experts).sum(dim=1).float()    # [B*T, num_experts]
        
        # calculate and combine experts outputs
        expert_outputs = torch.stack([expert(x_2d) for expert in self.experts], dim=1)       # [B*T, num_experts, C]    
        weighted_output = (expert_outputs * mask.unsqueeze(-1)).sum(dim=1).view(B, T, C)  # [B*T,C] -> [B,T,C]
        
        # balance load
        expert_mask = mask.mean(dim=0)   # [num_experts]
        router_prob = probs.mean(dim=0)  # [num_experts]
        self.loss = (expert_mask * router_prob).sum() * self.num_experts
        
        return self.dropout(weighted_output)


class Block(nn.Module):
    """ Transformer block: communication followed by computation """

    def __init__(self, n_embd, n_head, num_experts, topk_experts):
        # n_embd: embedding dimension, n_head: the number of heads we'd like
        super().__init__()
        head_size = n_embd // n_head
        self.sa = MultiHeadAttention(n_head, head_size)
        # self.ffwd = FeedForward(n_embd)
        self.moe = MoE(n_embd, num_experts, topk_experts)
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(n_embd, n_head, num_experts, topk_experts) for _ in range(n_layer)])
        self.ln_f = nn.LayerNorm(n_embd) # final layer norm
        self.lm_head = nn.Linear(n_embd, vocab_size)
        self.moe_coeff = moe_coeff
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is None:
            ...
            moe_loss = sum(block.moe.loss for block in self.blocks)

        return logits, loss, loss + self.moe_coeff * moe_loss
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            X, Y = get_batch(split)
            logits, loss, loss_with_moe = model(X, Y)
            losses[k] = loss.item()
...
def plot_losses(train_losses, val_losses, eval_interval=100, model_name="model"):
    ...
    plt.ylabel("Loss")
    plt.title(f"Training and validation losses for BigramLanguage model with MoE ({num_experts} experts, loss coefficient {moe_coeff})")
    plt.legend()
    
    plt.savefig(f"{dir_path}/loss_graphics/{model_name}.png")
...
model_name = f"moe_losscoeff{moe_coeff}_numexperts{num_experts}"
...
for iter in range(max_iters):
    ...
    logits, loss, loss_with_moe = model(xb, yb)

    loss_dynamics_train.append(loss.item())

    optimizer.zero_grad(set_to_none=True)
    loss_with_moe.backward()
    optimizer.step()
...
print("\nGeneration examples:")
generation_file = f"{dir_path}/generation_examples/{model_name}.txt"
context_strs = ["",
